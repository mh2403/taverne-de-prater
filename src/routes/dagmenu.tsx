    ...
    <SiteLayout>
      <section className="mx-auto max-w-4xl px-4 pt-16 md:px-6 md:pt-24">
        <p className="text-xs uppercase tracking-[0.25em] text-primary/70">
          Vandaag bij {data.business.name}
        </p>
        <h1 className="mt-2 font-serif text-4xl text-primary sm:text-5xl">
          {data.copy.dagmenuTitle}
        </h1>
        <p className="mt-4 text-foreground/75">{data.copy.dagmenuIntro}</p>
      </section>
        ...
        <div className="overflow-hidden rounded-3xl border border-border bg-card shadow-warm">
          <div className="bg-gradient-warm p-8 text-primary-foreground md:p-10">
            <div className="flex items-center justify-between">
              ...
              <span className="font-serif text-2xl text-gold">{data.dagmenu.price}</span>
            </div>
            <p className="mt-3 font-serif text-2xl">Driegangenmenu uit onze keuken</p>
          </div>
          <div className="divide-y divide-border p-8 md:p-10">
            <Course label="Soep / voorgerecht" value={data.dagmenu.soup} />
            <Course label="Hoofdgerecht" value={data.dagmenu.main} />
            <Course label="Dessert" value={data.dagmenu.dessert} />
          </div>
          <div className="border-t border-border bg-secondary/40 p-6 md:p-8">
            ...
            >
              <Phone className="h-4 w-4" /> Bel om te reserveren — {data.business.phone}
            </a>
          </div>
        </div>
      </section>
